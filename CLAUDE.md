...
## Business Model (Critical — applies everywhere)
- **Primary market:** Milwaukee Area
- **Physical hotels:** Waukesha and Brookfield, Wisconsin (Milwaukee Area)
- **Coverage:** Southeast Wisconsin and Chicagoland
- **Specialty:** Extended stays (7+ nights) and group bookings ONLY for partner hotels
- **Short stays (1–6 nights):** Search/API only — partner hotels NOT available for short stays
- **Welcome kit:** ALL extended stays (7+ nights) AND all group bookings. NEVER for short stays under 7 nights
- **No prices shown:** Rates are negotiated — never show specific nightly rates or dollar amounts anywhere on the site
...
- **Session 14 (2026-07-01):** Complete — Milwaukee Area primary positioning, extended stay specialty, Southeast Wisconsin & Chicagoland coverage. Home.tsx: hero "Extended Stay Hotel Specialist Milwaukee Area & Chicagoland", location bar, no prices, hotel cards single CTA "Get Extended Stay Rate →" with extended stay amenity tags, 4 detailed Who We Serve cards with type-param CTAs. Navbar: "Extended Stay & Groups" dropdown with 3 items (Extended Stay Quote / Group Booking / Search All Hotels). HotelCard.tsx: `twoButton` mode removes price + short stay button, shows fixed amenities tags + Milwaukee Area location text; new `viewOnly` prop for /exclusive page (no price, no button). ExclusiveHotels.tsx: updated hero, location context box (navy), "What Every Extended Stay Includes" section, URL param pre-fill (?type=nurse/crew/corporate/group), hotel cards viewOnly. GroupBooking.tsx: Milwaukee Area hero, 6 group benefits including welcome kit. About.tsx: full rewrite with Milwaukee Area lead, service area list. AI chat system prompt: full replacement — never quotes prices, collects 5 inquiry fields for extended stays, explains 35-min Kenosha drive, no welcome kit for short stays. email_service.py: welcome kit conditional on nights >= 7 in send_booking_confirmed, send_reservation_confirmed, send_pre_arrival_email; inquiry auto-reply updated. Confirmation.tsx: welcome kit step conditional on nights >= 7. index.html: SEO title and meta updated. CLAUDE.md: Business Model section added.
