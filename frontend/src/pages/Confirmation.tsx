  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        <CheckCircle2 className="w-12 h-12 mx-auto animate-pulse mb-4 text-slate-300" strokeWidth={1.5} />
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <p className="text-[#10192b] font-bold">{error}</p>
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-2xl mx-auto px-4 py-14">
        ...
        <div className="text-center mb-8">
          <Checkmark />
          <h1 className="font-display text-[#10192b] font-black text-3xl mt-6">Booking Received!</h1>
        ...
        {r.hotel_name_snapshot && (
          <div className="bg-white rounded-2xl shadow-sm p-6 mb-8">
            <h2 className="text-[#10192b] font-bold text-base mb-4">Booking Summary</h2>
            <div className="flex flex-col gap-3 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-500">Hotel</span>
                <span className="text-[#10192b] font-semibold text-right max-w-[55%]">{r.hotel_name_snapshot}</span>
              </div>
              {r.room_type_snapshot && (
                <div className="flex justify-between">
                  <span className="text-slate-500">Room</span>
                  <span className="text-[#10192b] font-semibold">{r.room_type_snapshot}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-slate-500">Check-in</span>
                <span className="text-[#10192b] font-semibold">{r.checkin_date}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Check-out</span>
                <span className="text-[#10192b] font-semibold">{r.checkout_date}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Nights</span>
                <span className="text-[#10192b] font-semibold">{r.nights}</span>
              </div>
              <div className="border-t border-slate-100 pt-3 flex justify-between">
                <span className="text-slate-500 font-semibold">Total (due at hotel)</span>
                <span className="text-[#10192b] font-black text-lg">${r.total_amount?.toFixed(0)}</span>
              </div>
              {r.card_last4 && (
                <div className="flex justify-between text-sm pt-1">
                  <span className="text-slate-500">Card guarantee</span>
                  <span className="text-[#10192b] font-semibold capitalize">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
          <h2 className="text-[#10192b] font-black text-lg mb-6">What happens in the next 30 minutes</h2>
          <div className="flex flex-col gap-6">
            {steps.map((step, i) => (
              <div key={i} className="flex gap-4">
                <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center
                  ${step.done ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-500'}`}>
                  <step.icon className="w-5 h-5" />
                </div>
                <div className="flex-1 pt-1">
                  <div className="flex items-center gap-2">
                    <h3 className={`font-bold text-sm ${step.done ? 'text-green-700' : 'text-[#10192b]'}`}>
                      {step.title}
                    </h3>
                    {step.done
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <a
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-[#10192b] hover:bg-[#0a1220] text-white font-bold py-3.5 px-6 rounded-xl text-sm transition-colors text-center"
          >
